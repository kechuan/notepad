...
>
>    旧浏览器的代表IE11 ES6 几乎只支持到let与const 
>
>    所以当你用到ES6的东西时
>
...
>
> 8. [!] 意味着这段内容在当前段落不合适 以后会去尝试迁移至其他符合的的地方 [?]意味着这段内容未完善 🕳
>
...
*let与var的作用域变量处理方式不一致 在for循环里 for中的var变量不会动 而let会*
...
> let [a,b,c] = [1,[7,42,4],10]



而ES6引入了这么种写法 这点和引入参数的**多项保留**有点类似
...
*[?]* [解构](https://www.cnblogs.com/tangjiao/p/9019671.html)
...
特别是json这样的**键**与**值** Map起到了比一般对象更好的管理作用
...
//浏览器应答adddlert is not defined
    ...
    finallyCode - 无论 try或catch 结果如何都会执行的代码块
}
```

> 不过说实话，你用它来调试有直接**F12**审查元素断点的效率高吗(存疑
>
> 也就演示页面会有点用吧



*try的作用域似乎是内置了另一种let 调用上层变量的时候需要用this*
...
那么`setTimeout`和`setInterval`的区别就是一个是延时且仅执行一次 
...
实例代码里 **setTimeout**确实也有进入到任务队列里啊

那为什么网上会说**setTimeout**是**同步** 而setInterval却是**异步**且性能更好的呢？
...
当然你取出结果也是要等主线程是了 否则主线程一个while(1)过来 该跪还得跪
...
promise需求你给出一个**基础函数** 以及 需求你传入一个/两个**回调函数**(callback)
...
> 
>
> //其中这里的value会默认指向Promise **fulfilled/resolved**状态之后reslove/reject之后返回过来的值
...
具体内容在npm里的then-fs上记述

promise的then方法本身就是一种链式调用

实际上 异步任务应该会是各个其余线程指派着去处理任务的

然而这样会导致返回结果的不统一(毕竟消息队列没有什么序号的概念 而且也不可能专门给你去做一个缓存区存放)

(这种需求在想异步操作dom时会特别频繁)



而promise的**then**方法恰好就是让该实例的promise函数全部只由一个非主线程执行 

这样就能避免出现不同步的问题

你当然也可以在promise里再new 一个promise出来 不清楚这样做会不会指派给另一个线程(
...
与java里的**catch**一样 一旦捕获到错误 就会让后面的**then**(try)语句停止执行
...
**throw**在**then**里像一个结尾 执行完之后等于屏蔽在其后的then执行
...
- 如果 **async** 函数内不含 **await** 表达式是会同步运行的



来看看[MDN](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Statements/async_function)的例子 来彻底解释async/await的工作流程罢(对应promise的全部特性)

```javascript
var resolveAfter2Seconds = function() {
  return new Promise(resolve=>{setTimeout(()=>{resolve(),console.log("slow")},2000)})
};

var resolveAfter1Seconds = function() {
  return new Promise(resolve=>{setTimeout(()=>{resolve(),console.log("fast")},1000)})
};
...
var sequentialStart = async function() {
  ...
  console.log(fast); //this runs (2+1) seconds
}
//因为await的加入 该线程必须要执行完await里的promise内容才会去继续执行接下来的任务
//与then类似的顺序函数执行
...
为什么这里的await还反而出现了并发的效果？

0. await本身就是一个等待序列(await特性第三点)
1. 直接await函数和间接await情况不一致
上述await的函数会使函数顺序占用单独线程执行
而此处却是并发将两个函数的值 赋给 变量之后
再去顺序输出值

上面的结果也可以看到 如果两个为顺序执行 fast是不会立马返回结果的
然而fast却在slow2s之后立马返回而不用等1s
说明fast早在slow等待1s时已经完成了那个函数的resolve解析 但是需要等待前面的await输出

造成了输出结果上的顺序执行 一般需求也大部分用的这种
...
说完Proxy 那就得再提提**Reflect**了

如果说Proxy是针对目标对象的任何操作的**全面检测(**Get, Set..)

那么Reflect就是专门针对目标对象的**操作行为**



实际上它们两个都是在ES6以前直接从Object内置方法内演化出来的

用来独立精简写法 以及更好的功能扩展



不过实际上它们两个都有对应的set/get

那为什么还要做一个重复的功能呢？



不是很清楚( 也许是两种风格的写法吧 你应该也可以捆绑写入
...
*在后续的ESNext中，Object的方法会被迁移到reflect当中*

> Reflect.getProptypeof()
